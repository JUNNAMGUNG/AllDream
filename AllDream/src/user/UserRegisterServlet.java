...
public class UserRegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("userID ============ UserRegisterServlet "+userID);
		...
		if(result == 1) {
			request.getSession().setAttribute("userID", userID);//회원가입시 자동으로 로그인
			request.getSession().setAttribute("messageType", "성공메세지");
			request.getSession().setAttribute("messageContent", "회원 가입에 성공 하셨습니다.");
